...
function weizhi_bianhua_gps_to_gaode_kaifazhe(zuobiao, num, str) {
  ...
  if (str == "gps_biaozhun") {
    ...
    zuobiao_then[1] =
      parseInt(zuobiao[0] / 100) + zuobiao_gps_yuanshen_ge_shi[0] / 60;
    zuobiao_then[0] =
      parseInt(zuobiao[1] / 100) + zuobiao_gps_yuanshen_ge_shi[1] / 60;
  }
  ...
  if (gaode_biaozhuni_is_yes == 0) {
    AMap.convertFrom(zuobiao_then, "gps", (status, result) => {
      if (result.info === "ok") {
        zhuobiao_zhuanhuan_R = result.locations[0].R;
        zhuobiao_zhuanhuan_Q = result.locations[0].Q;

        go_lubiao_all_kaifazhe[num] = [
          zhuobiao_zhuanhuan_R,
      ...
      success: function (result) {
        ...
        for (var i = 0; i < result.on_data.ID_len; i++) {
          weizhi_bianhua_gps_to_gaode_kaifazhe(
            [result.on_data.N[i], result.on_data.E[i]],
            i,
            result.on_data.str[i]
          );
          // console.log('路标已标注');
        }
        // 只 执行一次
...
// 自定义的起点 高德坐标系
kai_fa_zhe_qidian_zhuobiao_zhidingyi = [116.481686, 39.875344];

// 自定义的位置 高德坐标系
kai_fa_zhe_weizhi_zhuobiao_zhidingyi = [116.482298, 39.875199];
...
$("#test_kaifazhe").click(function () {
  ...
  if (kai_fa_zhe_is_yes_on == 0) {
    ...
    if (kaifazhe_bool == true) {
      ...
      zuobiao_to_qidian(
        kai_fa_zhe_qidian_zhuobiao_zhidingyi[0],
        kai_fa_zhe_qidian_zhuobiao_zhidingyi[1]
      );
      ...
      geocoder.getAddress(
        ...
        function (status, result) {
          if (status === "complete" && result.info === "OK") {
            // console.log(result);
            document.getElementById("qi_dian").innerHTML =
      ...
      geocoder.getAddress(
        [R_qidian_weizhi, Q_qidian_weizhi],
        function (status, result) {
          if (status === "complete" && result.info === "OK") {
            // console.log(result);
            document.getElementById("my_wei_zhi_dian").innerHTML =
              result.regeocode.formattedAddress;
          }
        }
      );
